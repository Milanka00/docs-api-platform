...
- **Payload size.** LLM request and response bodies are far larger than typical REST payloads, and body-inspecting policies buffer them in the runtime's memory. The ceiling multiplies by concurrency, so size the memory limit against both.
