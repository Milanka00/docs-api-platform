...
canonical_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/high-availability/
md_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/high-availability.md
tags:
  - ai-workspace
  - production
  - high-availability
...
On Kubernetes, that produces the following shape. Your ingress terminates TLS on port `443` and forwards to the AI Workspace Service on port `9643`, which balances across the workspace pods. Each workspace pod proxies to the Platform API Service on port `9243`, which balances across the Platform API pods, and every one of those pods connects to the same database on port `5432`. Gateways connect from outside the cluster to the same port `9243`, over a persistent WebSocket. Both workloads read their credentials from the Kubernetes Secrets created before install:

<!-- TODO: replace with the rendered diagram. Brief: diagrams/aiw-k8s-ha-diagram-spec.md -->
...
Each instance decrypts data the others wrote, and verifies tokens the others signed. That works only when they share identical key material:

- The same at-rest encryption key. A second instance with a different key can't read anything the first one stored.
- The same RS256 JWT keypair, if you run `file` or `internal_token` mode, so a token signed by one instance verifies at another. An `idp` deployment has no local keypair to match.

In Kubernetes this is automatic, because every pod reads one Secret. On a VM deployment across several hosts, copy the same files to each host and confirm they match before you send traffic. See [Provision secrets and keys](secrets-and-keys.md).
...
!!! warning "Configure refresh tokens before scaling the workspace"
    The BFF renews an expiring access token in the background, coordinated within one process. With several replicas, two of them can renew the same session at once. If your identity provider rotates refresh tokens on every use, one renewal invalidates the other's token and drops the session.

    Before you raise the workspace replica count, configure the provider to issue non-rotating refresh tokens for this client, or to allow a reuse grace window. Session affinity makes the collision unlikely, not impossible, because a pod restart or a rebalance moves a session.
