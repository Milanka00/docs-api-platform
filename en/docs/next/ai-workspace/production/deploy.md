...
canonical_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/deploy/
md_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/deploy.md
tags:
  - ai-workspace
  - production
  - deployment
...
!!! warning "On Kubernetes, write the driver as `postgres` exactly"
    The chart gates the HorizontalPodAutoscaler on a driver of `postgres`, matched literally. The aliases `postgresql` and `pgx` don't match it, and neither do `sqlite3` or `sqlserver`, so the autoscaler is left out of the release with no error. If you use any of those, set `replicaCount` yourself and scale manually. Step 10 depends on this.

**Before you continue:** connect to the database from the host or cluster that runs the Platform API, using the role you created. A connection that works from your laptop but not from the deployment is the most common reason a first install fails.
