...
canonical_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/secrets-and-keys/
md_url: https://wso2.com/api-platform/docs/next/ai-workspace/production/secrets-and-keys.md
tags:
  - ai-workspace
  - production
  - security
...
| RS256 JWT signing keypair | Platform API | Only in `file` and `internal_token` authentication modes. A production `idp` deployment doesn't need it | Nothing in `idp` mode. In the other modes, issued login tokens stop verifying and everyone signs in again |
...
!!! note "You don't need a JWT keypair in `idp` mode"
    Production delegates token validation to your identity provider, so the Platform API reads neither half of the keypair. This page doesn't ask you to manage one. Provision a keypair only if you run `file` or `internal_token` mode, where the Platform API signs and verifies its own tokens.
...
Step 1 covers the Secrets a production `idp` deployment needs. Two more key names exist for deployments that don't match that shape.

Add `ADMIN_USERNAME` and `ADMIN_PASSWORD_HASH` only if you keep file-based authentication. In OIDC mode the Platform API never reads them. Generate the hash with `htpasswd -inBC 12 "" | tr -d ':\n'`, which reads the password from standard input. The `-b` flag takes it as an argument instead, which puts it in the process list.

Add `jwt_public.pem`, and in `file` mode `jwt_private.pem` too, if you run `file` or `internal_token` authentication. Generate the pair with `openssl`, keep both halves together, and back up the private key: replacing it invalidates every issued token.

If you use an operator such as External Secrets Operator or the Secrets Store CSI Driver, have it produce a Secret with these key names. Then reference that Secret by name. The chart reads the Secret regardless of what created it.

## Step 3: Restrict who can read them

- Give the encryption key the narrowest audience that still lets the service start. On a VM that's file mode `600` owned by the account running the container runtime. In Kubernetes it's a Role that grants `get` on that Secret to the deployment's service account and to nobody else. Treat a JWT private key the same way if you run `file` or `internal_token` mode.
...
| JWT signing keypair, in `file` or `internal_token` mode | Replace both PEM files, restart the Platform API | Issued login tokens stop verifying, so users sign in again |
...
In `file` or `internal_token` mode, back up the JWT keypair the same way. An `idp` deployment has nothing else to keep.
